...
router.get("/", requireAuth, async (_req: Request, res: Response) => {
  try {
    if (isProd && supabase) {
      ...
      return res.status(500).json({ error: ERROR_CODES.SYSTEM[730] })
    }
  } catch (err: any) {
...
router.get("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    if (isProd && supabase) {
      ...
      if (error) throw error
      if (!data) return res.status(404).json({ error: ERROR_CODES.USERS[620] })
      ...
      if (!rows.length) return res.status(404).json({ error: ERROR_CODES.USERS[620] })
...
// ==============================
// ➕ POST /users  → Crear nuevo usuario (ya no genera tokens - usar /register en authRouter)
// ==============================
router.post("/", requireAuth, async (req: Request, res: Response) => {
  ...
  if (!nombre || !email || !password || rolId === undefined) {
    return res.status(400).json({
      ...
      message: "Faltan campos obligatorios: nombre, email, password o rolId",
    })
  }
  ...
  try {
    ...
    // Para registro público con tokens automáticos, usar /auth/register
...
router.put("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: ERROR_CODES.USERS[626] })
  }
})
